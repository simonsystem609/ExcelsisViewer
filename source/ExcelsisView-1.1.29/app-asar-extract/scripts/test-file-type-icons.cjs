...
assert.match(main, /const iconKind = options\.iconKind \|\| windowIconKind\(moduleName, fileSet\?\.path\)/);
assert.match(main, /moduleName === "dwg"\) return createDxfWindow\(fileSet, "dwg"\)/);
