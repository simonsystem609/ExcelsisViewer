        ...
        for (j = 0; j < num_tess_in_file; j++)
        {
            ...
            for (k = 0; k < num_parts; k++)
            {
                    ...
                    part->tess_file_index == i)
                {
                    ...
                    /* The duplicate-detail path above records this mapping,
                       but the first part that creates a new detail must record
                       it as well. Without it, every first-seen part retains
                       calloc's zero and consumers cannot map model-tree parts
                       back to the global tessellation table. */
                    reserve->parts[k].part_detail_index =
