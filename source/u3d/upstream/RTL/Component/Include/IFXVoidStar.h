...
class IFXBaseVoidHolder
	{
	public:
		uintptr_t	GetClassID(void) const			{ return m_classid; };
		uintptr_t	GetPointerClassID(void) const	{ return m_pointerclassid; };
...
virtual	IFXBaseVoidHolder *SubClone(U32 offset=0)=0;
	virtual ~IFXBaseVoidHolder() {}
		...
		void	SetPointerClassID(uintptr_t set)	{ m_pointerclassid=set; };

	protected:
		void	SetClassID(uintptr_t set)		{ m_classid=set; };
		void	SetRawPointer(void *set)		{ m_rawptr=set; };

	private:
		uintptr_t	m_classid;
		uintptr_t	m_pointerclassid;
		void	*m_rawptr;
	};
...
static	uintptr_t	GetStaticID(void)	{ return reinterpret_cast<uintptr_t>(&m_staticvalue); };
...
virtual			~IFXVoidStar(void)
		...
		IFXRESULT CopyPointerTo(T **tptr) const
					{
					uintptr_t voidT=IFXVoidHolder<void>::GetStaticID();
					uintptr_t staticT=IFXVoidHolder<T>::GetStaticID();
					if(!m_holder)
						{
		...
		bool	IsPointerToTypeOf(const IFXVoidStar &vs)
					...
					if(!m_holder || !vs.m_holder)
						return false;
					uintptr_t m_classid=m_holder->GetClassID();
					uintptr_t m_pointerclassid=vs.m_holder->GetPointerClassID();
