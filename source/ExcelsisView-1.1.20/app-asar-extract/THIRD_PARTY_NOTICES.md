...
ExcelsisView 1.1.20 is licensed at product level under the GNU Affero General
...
- nanoPRC, commit `66cacb70ac1072e9f23b677c5ae8dd83e61709c5`,
  AGPL-3.0-or-later. Its license and notices are installed under
  `modules/3dpdf/native/`; its modification record and exact corresponding
  source accompany the installer as `SOURCE-nanoPRC-66cacb70.zip`.
- U3D reference implementation, commit
  `5c141d9f0d366357e2b7cf93af2eade284a334be`, Apache-2.0. Its license
  is installed under `resources/third_party/licenses/u3d/`; its modification
  record, application bridge, and exact corresponding source accompany the
  installer as `SOURCE-U3D-5c141d9f.zip`.
- GNU LibreDWG 0.14.8492, commit
  `c34d1efb8fce8dfcd5bd7b4308c0d5e0b9de0a30`, GPL-3.0-or-later. Its license,
  and upstream notice are installed under `resources/third_party/libredwg/`;
  its exact source accompanies the installer as
...
- PDF.js 6.2.108, Apache-2.0. See `pdfjs-6.2.108.txt`. PDF.js also carries
...
- Three.js 0.160.0, MIT. See `three-0.160.0.txt`. Its object UUID generator is
  locally rebuilt to use the browser Web Crypto generator; the exact upstream
  source tarball and deterministic build modification accompany the installer
  in the application corresponding-source archive.
...
- buffer 6.0.3, MIT; ieee754 1.2.1, BSD-3-Clause; and base64-js 1.5.1, MIT.
  See the correspondingly named license files. Exact source tarballs for these
  packages and pako 2.1.0 are included in the application corresponding-source
  archive under `third_party/source/web-runtime/`.
...
- Electron 43.3.0, MIT. `LICENSE.electron.txt` is installed beside the
