  ...
  IFACEMETHODIMP GetThumbnail(UINT pixels, HBITMAP* bitmap, WTS_ALPHATYPE* alphaType) override {
    ...
    *alphaType = WTSAT_UNKNOWN;
    ...
    try {
      ...
      if (FAILED(result)) return result;
      if (StartsWith(data, "%PDF-", 5)) {
        result = RenderPdfThumbnail(data, pixels, bitmap, alphaType);
      } else if (StartsWith(data, "AC10", 4)) {
        result = RenderDwgThumbnail(data, pixels, bitmap);
        if (SUCCEEDED(result)) *alphaType = WTSAT_RGB;
      } else {
        result = excelsis::thumbnail::RenderDxfThumbnail(
            data.data(), data.size(), pixels, bitmap);
        if (SUCCEEDED(result)) *alphaType = WTSAT_RGB;
      }
      return result;
