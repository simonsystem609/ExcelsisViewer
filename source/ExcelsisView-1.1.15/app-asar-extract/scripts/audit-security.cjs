...
assert.equal(
  pkg.build.extraResources.some((entry) => String(entry.from) === "third_party/source"),
  false,
  "The installer still copies the unscoped corresponding-source directory.",
);
assert.match(
  sourcePackager,
...
assert.match(sourceDocument, /u3d-modified-source-5c141d9f\.zip/, "U3D corresponding source is not documented.");
