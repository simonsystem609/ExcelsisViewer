...
export function createRegularPdfViewer({ onModeChange, onStatus, onPathChange } = {}) {
  ...
  async function renderPage(entry, force = false) {
    ...
    const promise = (async () => {
      ...
      try {
        ...
        const pixelRatio = Math.min(devicePixelRatio || 1, 2, pixelRatioLimit);
  ...
  async function printDocument() {
    if (!documentHandle) return;
    setStatus(`Preparing ${documentHandle.numPages} page(s) for printing...`);
    for (const entry of pageEntries) await renderPage(entry, true);
