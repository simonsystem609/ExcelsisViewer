...
function createBatchPrintWindow(filePaths) {
  const entryUrl = applicationUrl("modules", "3dpdf", "batch-print.html");
  const win = new BrowserWindow({
    title: `${APP_NAME} Batch Print`,
    width: 1120,
    height: 780,
    minWidth: 900,
    minHeight: 650,
...
async function printOnePdf(owner, file, settings, batchIndex, batchTotal) {
  ...
  try {
    ...
    sendBatchPrintProgress(owner, {
      ...
      total: batchTotal,
      fileName: file.name,
      message: `Sending ${file.name} to the printer...`,
    });
  ...
  try {
    for (let index = 0; index < job.files.length; index += 1) {
      ...
      sendBatchPrintProgress(win, {
        ...
        fileName: file.name,
        message: `Preparing ${file.name} (${index + 1} of ${job.files.length})...`,
      });
    ...
    if (!win.isDestroyed()) {
      sendBatchPrintProgress(win, job.cancelRequested
        ? {
            ...
            index: 0,
            total: job.files.length,
            message: "Batch printing stopped before the next document.",
          }
        : {
            phase: "complete",
            index: job.files.length,
            total: job.files.length,
            message: `${job.files.length} PDF document(s) sent to the printer.`,
          });
    }
  } catch (error) {
    sendBatchPrintProgress(win, {
...
handleTrusted("batch-print:get-job", ["batch-print"], async (event) => {
  ...
  await requireBoundedFile(first.path, MAX_PDF_INPUT_BYTES, "Batch-print preview PDF");
  ...
  return {
    ...
    firstBytes: await fs.readFile(first.path),
...
handleTrusted("print-document:get-job", ["batch-print-document"], async (event) => {
  ...
  await requireBoundedFile(job.file.path, MAX_PDF_INPUT_BYTES, "Print PDF");
  return {
    bytes: await fs.readFile(job.file.path),
    label: job.file.name,
