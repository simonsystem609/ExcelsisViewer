...
import { showPdfTransformDialog } from "./pdf-transform-dialog.mjs";
...
export function createRegularPdfViewer({ onModeChange, onStatus, onPathChange, onStateChange } = {}) {
  ...
  const editor = createPdfEditor({
    ...
    onStatus: setStatus,
    onBusyChange: updateSaveControls,
  });
  ...
  function setSaveMenuOpen(open) {
    const nextOpen = !!open && !saveBusy && !transformBusy && !!documentHandle;
  ...
  function updateSaveControls() {
    ...
    saveMenuButton.disabled = saveBusy || transformBusy || !loaded;
    saveAsButton.disabled = saveBusy || transformBusy || !loaded;
    saveSplit.classList.toggle("saving", saveBusy);
    document.getElementById("pdfMirror").disabled = saveBusy || transformBusy || !loaded || editor.isBusy();
    document.getElementById("pdfScale").disabled = saveBusy || transformBusy || !loaded || editor.isBusy();
    onStateChange?.();
  }

  function setReadOnly(nextReadOnly) {
    readOnly = !!nextReadOnly;
    editor.setReadOnly(readOnly || transformBusy);
    updateSaveControls();
  }
  ...
  async function saveDocument({ saveAs = false } = {}) {
    if (!sourceBytes || saveBusy || transformBusy) return;
  ...
  async function transformDocument(kind) {
    if (!sourceBytes || !documentHandle || saveBusy || transformBusy || editor.isBusy()) return;
    transformBusy = true;
    setSaveMenuOpen(false);
    editor.setReadOnly(true); // Commit inline text and freeze edits while exporting.
    updateSaveControls();
    const viewState = { page: currentPage, rotation: 0, zoomMode };
    try {
      await showPdfTransformDialog({ kind, page: currentPage, pageCount: documentHandle.numPages, execute: async config => {
        const { transformPdfPages } = await import("./pdf-page-transform.mjs");
        const outputBytes = await transformPdfPages(await editor.exportBytes(), { ...config, viewRotation: rotation });
        const label = sourceLabel.replace(/\.pdf$/i, "") + (kind === "mirror" ? "-mirrored.pdf" : "-scaled.pdf");
        if (!desktopApi || !sourcePath) {
          downloadBytes(outputBytes, label);
          setStatus(`${label} downloaded; the original remains open.`);
          return true;
        }
        const result = await desktopApi.saveTransformedCopy(sourcePath, transferableBytes(outputBytes), label);
        if (result?.canceled) return false;
        if (!result?.ok) throw new Error(result?.error || "Could not save the transformed PDF.");
        onPathChange?.(result.path, result.lockState);
        await load(outputBytes, result.name, result.path, { viewState });
        setStatus(`${result.name} saved. Original unchanged.`);
        return true;
      }});
    } finally {
      transformBusy = false;
  ...
  async function printDocument() {
    if (!documentHandle || saveBusy || transformBusy) return;
  ...
  document.getElementById("pdfMirror").addEventListener("click", () => transformDocument("mirror").catch(error => setStatus(error.message)));
  document.getElementById("pdfScale").addEventListener("click", () => transformDocument("scale").catch(error => setStatus(error.message)));
  ...
  return {
    ...
    isBusy: () => saveBusy || transformBusy || editor.isBusy(),
    setFileIdentity(filePath, label) {
      sourcePath = filePath;
      sourceLabel = label;
      updateSaveControls();
      setStatus(documentStatus());
    ...
    async discard() {
      ...
      const bytes = sourcePath && desktopApi?.readFile ? await desktopApi.readFile(sourcePath) : bytesFrom(sourceBytes);
      await load(bytes, sourceLabel, sourcePath, { viewState: { page: currentPage, rotation, zoomMode } });
    },
  };
}
