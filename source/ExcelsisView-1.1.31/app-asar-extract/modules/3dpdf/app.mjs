...
async function handleFile(file){
  if(fileActionBusy||fileLoadBusy||regularPdfViewer?.isBusy())return;
...
function syncPdfFileControls(){
  const unavailable=!currentDesktopPath||fileActionBusy||fileLoadBusy||regularPdfViewer?.isBusy();
...
async function managePdfFile(kind){
  if(!currentDesktopPath||fileActionBusy||fileLoadBusy||regularPdfViewer?.isBusy())return;
...
async function decodeDesktopFile(fileInfo){
  ...
  finally{fileLoadBusy=false;syncPdfFileControls();}
}
