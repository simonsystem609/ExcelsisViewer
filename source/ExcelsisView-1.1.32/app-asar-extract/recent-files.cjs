...
function createRecentFiles({ read, write, now = Date.now }) {
  ...
  function forget(filePath) {
    if (!sectionForPath(filePath)) return Promise.resolve(false);
    pending = pending.then(async () => {
      entries = entries.filter((entry) => recentId(entry.path) !== recentId(filePath));
      try {
        await write(JSON.stringify({ version: 1, entries }));
        warning = "";
      } catch {
        warning = "Recent files are available for this session, but their history could not be saved.";
      }
  ...
  return { list, remember, resolve, forget };
}
