...
let fileActionBusy=false,fileLoadBusy=false;
let fileNavigationBusy=false,pdfFolderFiles=[];
...
async function handleFile(file){
  if(fileNavigationBusy||fileActionBusy||fileLoadBusy||regularPdfViewer?.isBusy())return;
...
function syncPdfFileControls(){
  const unavailable=!currentDesktopPath||fileNavigationBusy||fileActionBusy||fileLoadBusy||regularPdfViewer?.isBusy();
  const index=pdfFolderFiles.findIndex(file=>file.path.toLowerCase()===currentDesktopPath?.toLowerCase());
  document.getElementById('pdfPrevFile').disabled=!!unavailable||index<=0;
  document.getElementById('pdfNextFile').disabled=!!unavailable||index<0||index>=pdfFolderFiles.length-1;
  document.getElementById('pdfRenameFile').disabled=!!(unavailable||currentReadOnly);
  document.getElementById('pdfDeleteFile').disabled=!!(unavailable||currentReadOnly);
  document.getElementById('pdfDiscard').disabled=!!unavailable;
}
async function managePdfFile(kind){
  if(!currentDesktopPath||fileNavigationBusy||fileActionBusy||fileLoadBusy||regularPdfViewer?.isBusy())return;
  if(kind!=='discard'&&currentReadOnly)return;
  const originalPath=currentDesktopPath;
  const name=fileNameFromPath(originalPath);
  fileActionBusy=true;
  syncPdfFileControls();
  try{
    await confirmFileAction({kind,name,path:originalPath,dirty:!!regularPdfViewer?.isDirty(),execute:async newName=>{
      if(currentDesktopPath!==originalPath)throw new Error('The open PDF changed. Reopen the confirmation.');
      if(kind==='discard'){
        if(!pdfWorkspace.hidden)await regularPdfViewer.discard();
        else await decodeDesktopFile({path:originalPath,name});
        return {ok:true};
      }
      const result=kind==='rename'?await desktopApi.renameFile(originalPath,newName):await desktopApi.deleteFile(originalPath);
      if(!result?.ok)throw new Error(result?.error||'The file operation did not complete.');
      if(kind==='rename'){
        currentDesktopPath=result.path;
        regularPdfViewer?.setFileIdentity(result.path,result.name);
        setLockState(result.lockState||{readOnly:false});
        await refreshPdfFolder();
        stats.textContent=`${result.name} renamed.`;
      }else{
        currentDesktopPath=null;
        if(regularPdfViewer)await regularPdfViewer.clear();
        if(modelRoot)disposeModel();
        setViewerMode('three');
        cv.style.display='none';hint.style.display='';
        setLockState({readOnly:false});
        stats.textContent=`${name} moved to the Recycle Bin.`;
      }
      return result;
    }});
  }finally{fileActionBusy=false;syncPdfFileControls();}
}
document.getElementById('pdfRenameFile').addEventListener('click',()=>managePdfFile('rename'));
document.getElementById('pdfDeleteFile').addEventListener('click',()=>managePdfFile('delete'));
document.getElementById('pdfDiscard').addEventListener('click',()=>managePdfFile('discard'));
async function refreshPdfFolder(){
  const source=currentDesktopPath;
  if(!source||!desktopApi?.listFolder){pdfFolderFiles=[];syncPdfFileControls();return;}
  const set=await desktopApi.listFolder(source);
  if(currentDesktopPath!==source)return;
  pdfFolderFiles=set.files||[];
  syncPdfFileControls();
}
async function goPdfFile(delta){
  if(!currentDesktopPath||fileNavigationBusy||fileActionBusy||fileLoadBusy||regularPdfViewer?.isBusy())return;
  fileNavigationBusy=true;syncPdfFileControls();
  try{
    await refreshPdfFolder();
    const index=pdfFolderFiles.findIndex(file=>file.path.toLowerCase()===currentDesktopPath.toLowerCase());
    if(index<0)return;
    let next=index+delta;
    while(next>=0&&next<pdfFolderFiles.length&&await desktopApi.isFileOpenElsewhere(pdfFolderFiles[next].path))next+=delta;
    if(next<0||next>=pdfFolderFiles.length)return;
    if(regularPdfViewer?.isDirty()){
      if(!confirm('Save changes before changing file? OK saves and continues. Cancel stays here.'))return;
      await regularPdfViewer.save();
      if(regularPdfViewer.isDirty())return;
    }
    await decodeDesktopFile(pdfFolderFiles[next]);
  }catch(error){stats.textContent='Could not change file: '+(error.message||error);}
  finally{fileNavigationBusy=false;syncPdfFileControls();}
}
document.getElementById('pdfPrevFile').addEventListener('click',()=>goPdfFile(-1));
document.getElementById('pdfNextFile').addEventListener('click',()=>goPdfFile(1));
bindFolderNavigation({
  previous:document.getElementById('pdfPrevFile'),next:document.getElementById('pdfNextFile'),
  isBlocked:()=>fileNavigationBusy||fileActionBusy||fileLoadBusy||regularPdfViewer?.isBusy(),
});
addEventListener('focus',()=>{if(!fileNavigationBusy&&!fileActionBusy&&!fileLoadBusy)refreshPdfFolder().catch(()=>{});});
...
async function decodeDesktopFile(fileInfo){
  ...
  try{
    ...
    try{await refreshPdfFolder();}catch(error){console.warn('Could not refresh PDF folder',error);}
    fileLoadBusy=false;syncPdfFileControls();
  }
}
...
async function initDesktopBridge(){
  ...
  desktopApi.onFileSaved(async savedState=>{if(savedState?.path===currentDesktopPath&&!fileActionBusy&&!regularPdfViewer?.isBusy())await decodeDesktopFile({path:currentDesktopPath,name:fileNameFromPath(currentDesktopPath)});});
  try{
