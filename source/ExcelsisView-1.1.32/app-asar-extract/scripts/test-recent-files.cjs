...
async function run() {
  ...
  await store.forget(file("69.DXF"));
  assert.equal((await store.list("dxf")).entries.some((entry) => entry.name === "69.dxf"), false);
  await store.forget(file("one.dwg"));
  await store.remember(file("renamed.dwg"));
  assert.equal((await store.list("dwg")).entries.some((entry) => entry.name === "renamed.dwg"), true);
