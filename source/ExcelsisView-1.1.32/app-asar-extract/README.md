...
ExcelsisView 1.1.32 is a Windows viewer for DXF, DWG, regular PDF, and 3D PDF
documents.

Left/Right arrow keys navigate the previous/next file in the same folder in
DXF, DWG and PDF windows. PDF has separate Prev file / Next file controls;
its page buttons are unchanged. Editing fields and open dialogs keep their
arrow keys. Modified/held keys are ignored, in-progress actions block navigation,
and unsaved edits require saving before switching (Cancel stays on the file).

Version 1.1.32 adds Mirror and Scale to regular PDF pages. Mirror flips left/right
