...
contextBridge.exposeInMainWorld("excelsisRecents", {
  list: () => ipcRenderer.invoke("recents:list"),
  open: (id) => ipcRenderer.invoke("recents:open", id),
  onChanged: (callback) => onAppChannel("recents:changed", callback),
});
...
contextBridge.exposeInMainWorld("dxfApp", {
  ...
  renameFile: (filePath, newName) => ipcRenderer.invoke("fs:rename-current-file", filePath, newName),
  deleteFile: (filePath) => ipcRenderer.invoke("fs:delete-current-file", filePath),
  onFilesChanged: (callback) => onAppChannel("app:files-changed", callback),
...
contextBridge.exposeInMainWorld("pdfApp", {
  ...
  listFolder: (filePath) => ipcRenderer.invoke("fs:list-pdf-folder", filePath),
  isFileOpenElsewhere: (filePath) => ipcRenderer.invoke("fs:is-file-open-elsewhere", filePath),
  renameFile: (filePath, newName) => ipcRenderer.invoke("fs:rename-current-file", filePath, newName),
  deleteFile: (filePath) => ipcRenderer.invoke("fs:delete-current-file", filePath),
  ...
  openPrintPreview: (filePath, bytes, label) => ipcRenderer.invoke(
    "print:open-preview",
    filePath,
    bytes,
    label,
  ...
  saveTransformedCopy: (filePath, bytes, suggestedName) => ipcRenderer.invoke(
    "fs:save-pdf-transformed-copy", filePath, bytes, suggestedName,
  ),
  getPathForFile: grantPathForFile,
  openFileInWindow: (filePath) => ipcRenderer.invoke("app:open-file-in-window", filePath),
  onFileState: (callback) => onAppChannel("app:file-state", callback),
