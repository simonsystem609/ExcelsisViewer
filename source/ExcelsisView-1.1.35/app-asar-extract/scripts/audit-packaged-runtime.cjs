...
const EXPECTED_CHROMIUM_VERSION = "150.0.7871.212";
...
async function main() {
  ...
  const expectedPdfJsVersion = sourceMetadata.devDependencies?.["pdfjs-dist"];
  ...
  assert.equal(expectedPdfJsVersion, "6.2.108", "The patched PDF.js version is not pinned.");
  ...
  const packagedPdfJs = asar.extractFile(
    asarPath,
    path.join("modules", "3dpdf", "vendor", "pdfjs", "pdf.min.mjs"),
  ).toString("utf8");
  assert.match(
    packagedPdfJs,
    new RegExp(`pdfjsVersion = ${expectedPdfJsVersion.replaceAll(".", "\\.")}`),
    "Packaged PDF.js runtime does not match the patched pin.",
  );
  assert.equal(
  ...
  const packagedPreload = asar.extractFile(asarPath, "preload.cjs").toString("utf8");
  for (const relativePath of [
    ...
    "modules/3dpdf/pdf-page-transform.mjs", "modules/3dpdf/pdf-transform-dialog.mjs",
    "modules/3dpdf/pdf-editor.mjs", "modules/3dpdf/styles.css",
    "modules/3dpdf/vendor/pdf-edit/pdf-export-runtime.mjs",
  ]) {
    assert.deepEqual(
      asar.extractFile(asarPath, path.join(...relativePath.split("/"))),
      fs.readFileSync(path.join(projectRoot, ...relativePath.split("/"))),
      `Packaged feature bytes differ from source: ${relativePath}`,
    );
  }
  const packagedUpdaterUtils = asar.extractFile(asarPath, "update-center-utils.cjs").toString("utf8");
  const packagedUpdateCenter = asar.extractFile(
    asarPath,
    path.join("launcher", "update-center.html"),
  ).toString("utf8");
  const packagedUpdateCenterScript = asar.extractFile(
    asarPath,
    path.join("launcher", "update-center.js"),
  ).toString("utf8");
  ...
  assert.match(packagedMain, /app\.getPath\("downloads"\)/, "Packaged updater does not target Downloads.");
  assert.match(packagedMain, /shell\.openPath\(installerPath\)/, "Packaged updater launch is missing.");
  assert.match(packagedPreload, /install: \(productKey\).*update:download-and-run/);
  assert.doesNotMatch(packagedPreload, /downloadUrl|installerPath/);
  assert.match(packagedUpdaterUtils, /repository: "ExcelsisViewer"/);
  assert.match(packagedUpdaterUtils, /repository: "ExcelsisHelper"/);
  ...
  assert.match(packagedUpdateCenterScript, /!update\.canInstall/);
  assert.match(packagedMain, /registryQueryExecutable\(\), \[/);
  assert.match(packagedMain, /is newer than the latest public version/);
  ...
  const adjacentAppSource = path.join(
    releaseOutput,
    `SOURCE-ExcelsisView-${expectedVersion}.zip`,
  );
  const adjacentNanoSource = path.join(releaseOutput, "SOURCE-nanoPRC-66cacb70.zip");
  const adjacentU3dSource = path.join(releaseOutput, "SOURCE-U3D-5c141d9f.zip");
  const adjacentLibreDwgSource = path.join(
    releaseOutput,
  ...
  const requiredResourceFiles = [
    ...
    path.join("build", "icon-dxf-256.png"),
    path.join("build", "icon-dxf.ico"),
    path.join("build", "icon-file-dxf-256.png"),
    path.join("build", "icon-file-dxf.ico"),
    path.join("build", "icon-file-dwg-256.png"),
    path.join("build", "icon-file-dwg.ico"),
    path.join("build", "icon-file-pdf-256.png"),
    path.join("build", "icon-file-pdf.ico"),
    ...
    path.join("third_party", "licenses", "pdfjs-6.2.108.txt"),
  ...
  for (const forbiddenBundledSource of [
    path.join(resourcesPath, "third_party", "source"),
    path.join(resourcesPath, "third_party", "libredwg", "libredwg-0.14.8492-source.tar.gz"),
  ]) {
    assert.equal(
      fs.existsSync(forbiddenBundledSource),
      false,
      `Standalone runtime still nests corresponding source: ${forbiddenBundledSource}`,
    );
  }
  ...
  assert.equal(
    crypto.createHash("sha256").update(fs.readFileSync(adjacentLibreDwgSource)).digest("hex"),
    "9935245817278c944c681527ef52eee81ccf720fce09f9fb467d0d6a926ae3ce",
    "Installer-adjacent LibreDWG corresponding source has the wrong hash.",
  );
  ...
  const nanoArchiveEntries = execFileSync("tar.exe", ["-tf", adjacentNanoSource], {
    encoding: "utf8",
  });
  ...
  const u3dArchiveEntries = execFileSync("tar.exe", ["-tf", adjacentU3dSource], {
    encoding: "utf8",
  }).split(/\r?\n/);
  for (const expectedEntry of [
    "UPSTREAM-COMMIT.txt",
    ...
    "upstream/COPYING",
    "upstream/RTL/Component/Bones/CIFXAnimationModifier.cpp",
    "upstream/RTL/Component/Include/IFXVoidStar.h",
    "upstream/RTL/Component/Subdiv/IFXTQTBaseTriangle.h",
    "upstream/RTL/Component/Texture/CIFXImageTools.cpp",
    "upstream/RTL/Component/Texture/CIFXTextureImageTools.cpp",
    "upstream/RTL/Component/Texture/IFXImageSizeUtils.h",
    ...
    "upstream/Samples/TestScenes/ColorChannels.u3d",
    "upstream/Samples/TestScenes/shading_alpha.u3d",
    "tests/image_size_utils_test.cpp",
  ]) {
    assert.ok(
  ...
  const appArchiveEntries = execFileSync("tar.exe", ["-tf", adjacentAppSource], {
    encoding: "utf8",
  }).split(/\r?\n/);
  ...
  for (const expectedEntry of [
    `${appPrefix}/main.cjs`,
    `${appPrefix}/recent-files.cjs`,
    `${appPrefix}/file-actions.cjs`,
    `${appPrefix}/modules/shared/file-action-dialog.mjs`,
    `${appPrefix}/modules/shared/file-action-dialog.css`,
    `${appPrefix}/scripts/test-file-actions.cjs`,
    `${appPrefix}/scripts/test-file-actions-renderer.cjs`,
    `${appPrefix}/scripts/test-folder-navigation.cjs`,
    `${appPrefix}/network-thumbnail-policy.cjs`,
    `${appPrefix}/scripts/test-network-thumbnail-policy.cjs`,
    `${appPrefix}/shell/thumbnail-provider/src/network_thumbnail_policy.h`,
    `${appPrefix}/shell/thumbnail-provider/tests/network_thumbnail_policy_tests.h`,
    `${appPrefix}/modules/shared/folder-navigation.mjs`,
    `${appPrefix}/modules/3dpdf/pdf-page-transform.mjs`,
    `${appPrefix}/modules/3dpdf/pdf-transform-dialog.mjs`,
    `${appPrefix}/scripts/test-pdf-page-transform.mjs`,
    `${appPrefix}/scripts/test-pdf-transform-renderer.cjs`,
    `${appPrefix}/modules/shared/recents.mjs`,
    `${appPrefix}/modules/shared/recents.css`,
    `${appPrefix}/modules/dxf/racetrack.mjs`,
    `${appPrefix}/modules/dxf/racetrack-dialog.mjs`,
    ...
    `${appPrefix}/scripts/test-u3d-hardening.cjs`,
    ...
    `${sourcePrefix}/u3d/tests/image_size_utils_test.cpp`,
    `${sourcePrefix}/u3d/upstream/RTL/Component/Bones/CIFXAnimationModifier.cpp`,
    `${sourcePrefix}/u3d/upstream/RTL/Component/Include/IFXVoidStar.h`,
    `${sourcePrefix}/u3d/upstream/RTL/Component/Subdiv/IFXTQTBaseTriangle.h`,
    `${sourcePrefix}/u3d/upstream/RTL/Component/Texture/IFXImageSizeUtils.h`,
  ...
  for (const [releaseName, referenceSource] of [
    [
      `SOURCE-ExcelsisView-${expectedVersion}.zip`,
      path.join(
        projectRoot,
    ...
    [
      "SOURCE-LibreDWG-0.14.8492.tar.gz",
      path.join(projectRoot, "third_party", "libredwg", "libredwg-0.14.8492-source.tar.gz"),
    ],
    ...
    assert.equal(
      ...
      crypto.createHash("sha256").update(fs.readFileSync(referenceSource)).digest("hex"),
      `Installer-adjacent material differs from its exact source: ${releaseName}`,
    );
  }
  console.log(JSON.stringify({
    ...
    nanoPrc: {
      ...
      sourceArchivePath: adjacentNanoSource,
    },
    u3d: {
      exporterPath: packagedU3dExporter,
      sourceArchivePath: adjacentU3dSource,
