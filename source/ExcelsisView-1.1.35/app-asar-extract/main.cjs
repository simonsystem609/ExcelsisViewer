...
const { createRecentFiles, MAX_HISTORY_BYTES } = require("./recent-files.cjs");
const { createFileActions } = require("./file-actions.cjs");
...
const MAX_UPDATE_API_BYTES = 2 * 1024 * 1024;
...
const MODULES = new Set(["dxf", "dwg", "3dpdf", "update-center"]);
...
const updateInstallTasks = new Map();
const fileActions = createFileActions({
  assertCurrent: assertCurrentFileAction,
...
function windowIconKind(moduleName, filePath = null) {
  if (moduleName === "launcher" || moduleName === "update-center") return "launcher";
...
function updateCenterEntryUrl() {
  const entryPath = path.join(__dirname, "launcher", "update-center.html");
  if (!fsSync.existsSync(entryPath)) return null;
  return applicationUrl("launcher", "update-center.html");
}
...
function requireFileCapability(event, filePath, expectedModule) {
  ...
  if (fileActions.isBusy(event.sender.id, resolved)) throw new Error("A file operation is in progress.");
...
function createModuleWindow(moduleName, fileSet = null, options = {}) {
  const isLauncher = moduleName === "launcher";
  const isUpdateCenter = moduleName === "update-center";
  const isCompactWindow = isLauncher || isUpdateCenter;
  const entryUrl = isLauncher
    ? launcherEntryUrl()
    : (isUpdateCenter ? updateCenterEntryUrl() : moduleEntryUrl(moduleName));
  if (!entryUrl) throw new Error(`Module not found: ${moduleName}`);
  const iconKind = options.iconKind || windowIconKind(moduleName, fileSet?.path);

  const win = new BrowserWindow({
    title: isLauncher
      ? `${APP_NAME} Launcher`
      : (isUpdateCenter ? `${APP_NAME} Update Center` : APP_NAME),
    width: isLauncher ? 720 : (isUpdateCenter ? 780 : 1240),
    height: isLauncher ? 520 : (isUpdateCenter ? 620 : 820),
    minWidth: isCompactWindow ? 560 : 900,
    minHeight: isCompactWindow ? 420 : 620,
    autoHideMenuBar: true,
    backgroundColor: isCompactWindow ? "#f4f7fb" : "#050607",
...
function createDxfWindow(fileSet = null, iconKind = null) {
  return createModuleWindow("dxf", fileSet, iconKind ? { iconKind } : {});
}
...
function showUpdateCenterWindow() {
  const existing = BrowserWindow.getAllWindows().find(
    (win) => win.excelsisModuleName === "update-center",
  );
  if (existing) {
    existing.show();
    existing.focus();
...
function findExistingWindowForFile(filePath) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (!claim && pending?.path && normalizePath(pending.path) === normalized) return win;
  }
  return null;
}
...
function openHttpsResponse(rawUrl, headers) {
  return new Promise((resolve, reject) => {
    const request = https.get(rawUrl, {
      headers,
      timeout: 30000,
    }, resolve);
    request.once("timeout", () => request.destroy(new Error("The update server timed out.")));
    request.once("error", reject);
  });
}

async function readBoundedHttpsBody(response, byteLimit) {
  const declaredLength = Number(response.headers["content-length"] || 0);
  if (declaredLength && (!Number.isSafeInteger(declaredLength) || declaredLength > byteLimit)) {
    response.destroy();
    throw new Error("The update response exceeds its safety limit.");
  }
  const chunks = [];
  let total = 0;
...
async function fetchLatestUpdate(productKey) {
  const response = await openHttpsResponse(latestReleaseApiUrl(productKey), {
  ...
  if (response.statusCode !== 200) {
    response.resume();
    throw new Error(`GitHub release lookup failed with HTTP ${response.statusCode || "unknown"}.`);
  }
...
function registryQueryExecutable() {
  ...
  if (!fsSync.existsSync(executablePath)) {
    throw new Error("Windows Registry query support is unavailable.");
  }
  return executablePath;
}

async function installedProductVersion(productKey) {
  const product = productForKey(productKey);
  if (process.platform !== "win32") {
    return product.key === "viewer" ? app.getVersion() : null;
  }
  const registryKey =
    `HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\${product.uninstallGuid}`;
  for (const registryView of ["64", "32"]) {
    try {
      const { stdout } = await execFileAsync(registryQueryExecutable(), [
        "query",
        registryKey,
        "/v",
        "DisplayVersion",
      ...
      const version = /^\s*DisplayVersion\s+REG_\w+\s+(.+?)\s*$/mi.exec(stdout)?.[1]?.trim();
      if (!version || version.length > 80 || !/^[0-9A-Za-z._-]+$/.test(version)) {
        throw new Error(`Windows returned an invalid ${product.displayName} installed version.`);
      }
      return version;
    } catch (error) {
      if (error?.code === 1) continue;
      throw new Error(`Could not read the installed ${product.displayName} version.`);
    }
  }
  // A packaged Viewer can still identify itself even if its uninstall entry
  // was removed manually. Helper has no equivalent trusted runtime fallback.
  return product.key === "viewer" && app.isPackaged ? app.getVersion() : null;
}

function publicUpdateInfo(update, installedVersion, installedVersionError = null) {
  const status = installedVersionError
    ? "unknown"
    : updateStatusForVersions(installedVersion, update.version);
  return {
    ...
    status,
    canInstall: status === "not-installed" || status === "update-available",
...
async function updateCatalog() {
  const [products, macros] = await Promise.all([
    ...
    try {
      installedVersion = await installedProductVersion(productKey);
    } catch (error) {
      installedVersionError = error?.message || "Could not read the installed version.";
    }
    try {
      return publicUpdateInfo(
        await fetchLatestUpdate(productKey),
        installedVersion,
        installedVersionError,
      );
    } catch (error) {
...
async function fileHasExpectedInstaller(filePath, asset) {
  let stat;
  try {
  ...
  try {
    ...
    if (bytesRead !== 2 || signature.toString("ascii") !== "MZ") return false;
  } finally {
    await handle.close();
  }
...
function numberedInstallerPath(downloadsPath, installerName, index) {
  if (index === 0) return path.join(downloadsPath, installerName);
  const parsed = path.parse(installerName);
  return path.join(downloadsPath, `${parsed.name} (${index})${parsed.ext}`);
}

async function findExistingVerifiedInstaller(downloadsPath, asset) {
  for (let index = 0; index <= 20; index += 1) {
    const candidatePath = numberedInstallerPath(downloadsPath, asset.name, index);
    if (await fileHasExpectedInstaller(candidatePath, asset)) return candidatePath;
  }
  return null;
}

async function openInstallerDownloadResponse(assetUrl) {
  let currentUrl = assetUrl;
  for (let redirects = 0; redirects <= 5; redirects += 1) {
    if (!isAllowedFinalDownloadUrl(currentUrl)) {
      throw new Error("GitHub redirected the installer to an unapproved host.");
    }
    const response = await openHttpsResponse(currentUrl, {
    ...
    if ([301, 302, 303, 307, 308].includes(response.statusCode)) {
      ...
      response.resume();
      if (!location) throw new Error("GitHub returned an invalid installer redirect.");
      currentUrl = new URL(location, currentUrl).href;
      continue;
    }
    if (response.statusCode !== 200) {
      response.resume();
      throw new Error(`Installer download failed with HTTP ${response.statusCode || "unknown"}.`);
    }
    return response;
  }
  throw new Error("The installer download used too many redirects.");
}

async function downloadInstallerToPart(event, update, partPath) {
  const response = await openInstallerDownloadResponse(update.asset.downloadUrl);
  const declaredLength = Number(response.headers["content-length"] || 0);
  if (declaredLength && declaredLength !== update.asset.size) {
    response.destroy();
    throw new Error("The installer size does not match the immutable release metadata.");
  }
  ...
  try {
    for await (const value of response) {
      const chunk = Buffer.from(value);
      total += chunk.length;
      if (total > update.asset.size || total > MAX_INSTALLER_BYTES) {
        response.destroy();
        throw new Error("The installer download exceeded its verified size.");
      }
      await handle.write(chunk);
      const percent = Math.floor((total / update.asset.size) * 100);
      if (percent !== lastPercent) {
        ...
        sendUpdateProgress(
          ...
          update.productKey,
          percent,
          `Downloading ${update.asset.name} - ${percent}%`,
        );
      }
    }
    await handle.sync();
  ...
  if (total !== update.asset.size) {
    throw new Error("The installer download ended before its verified size was reached.");
  }
}

async function publishVerifiedInstaller(partPath, downloadsPath, asset) {
  for (let index = 0; index <= 200; index += 1) {
    const targetPath = numberedInstallerPath(downloadsPath, asset.name, index);
    try {
      await fs.copyFile(partPath, targetPath, fsSync.constants.COPYFILE_EXCL);
    } catch (error) {
      if (error?.code !== "EEXIST") throw error;
      if (await fileHasExpectedInstaller(targetPath, asset)) return targetPath;
      continue;
    }
    if (!await fileHasExpectedInstaller(targetPath, asset)) {
      await fs.rm(targetPath, { force: true });
      throw new Error("The installer changed while it was copied into Downloads.");
    }
    return targetPath;
  }
  throw new Error("Downloads contains too many installers with the same name.");
}

async function downloadAndRunUpdate(event, productKey) {
  if (updateInstallTasks.has(productKey)) {
    throw new Error("That installer is already being downloaded.");
  }
  const task = (async () => {
    const update = await fetchLatestUpdate(productKey);
    const installedVersion = await installedProductVersion(productKey);
    const updateStatus = updateStatusForVersions(installedVersion, update.version);
    if (updateStatus === "up-to-date") {
      throw new Error(`${update.displayName} ${installedVersion} is already up to date.`);
    }
    if (updateStatus === "newer-installed") {
      throw new Error(
        `${update.displayName} ${installedVersion} is newer than the latest public version ${update.version}.`,
      );
    }
    if (updateStatus === "unknown") {
      throw new Error(`The installed ${update.displayName} version cannot be compared safely.`);
    }
    const downloadsPath = app.getPath("downloads");
    await fs.mkdir(downloadsPath, { recursive: true });
    sendUpdateProgress(event, productKey, 0, "Checking Downloads for a verified copy...");
    let installerPath = await findExistingVerifiedInstaller(downloadsPath, update.asset);
    if (!installerPath) {
      const partPath = path.join(
        downloadsPath,
        `.${update.asset.name}.${crypto.randomUUID()}.part`,
      );
      try {
        await downloadInstallerToPart(event, update, partPath);
        sendUpdateProgress(event, productKey, 100, "Verifying SHA-256 digest...");
        if (!await fileHasExpectedInstaller(partPath, update.asset)) {
          throw new Error("The downloaded installer failed its SHA-256 or PE verification.");
        }
        installerPath = await publishVerifiedInstaller(partPath, downloadsPath, update.asset);
      } finally {
        await fs.rm(partPath, { force: true }).catch(() => {});
      }
    } else {
      sendUpdateProgress(event, productKey, 100, "Using the verified installer already in Downloads.");
    }
    if (!await fileHasExpectedInstaller(installerPath, update.asset)) {
      throw new Error("The installer failed verification immediately before launch.");
    }
    const openError = await shell.openPath(installerPath);
    if (openError) throw new Error(`Windows could not open the installer: ${openError}`);
    sendUpdateProgress(event, productKey, 100, "Verified installer opened.");
    return { ok: true, path: installerPath, tag: update.tag };
  })();
  updateInstallTasks.set(productKey, task);
  try {
    return await task;
  } finally {
...
function getRecentFiles() {
  if (!recentFiles) {
    const historyPath = path.join(app.getPath("userData"), "RecentFiles.json");
    recentFiles = createRecentFiles({
      read: async () => {
        try {
          await requireBoundedFile(historyPath, MAX_HISTORY_BYTES, "Recent-file history");
      ...
      write: async (text) => {
        ...
        await fs.writeFile(`${historyPath}.tmp`, text, { encoding: "utf8", mode: 0o600 });
...
async function rememberRecentFile(filePath) {
  await getRecentFiles().remember(filePath);
  broadcastRecentFilesChanged();
}

function broadcastRecentFilesChanged() {
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed() && ["dxf", "3dpdf"].includes(win.excelsisModuleName)) {
      win.webContents.send("recents:changed");
    }
  }
}

function recentSectionForSender(event) {
  const win = BrowserWindow.fromWebContents(event.sender);
  // A DWG window uses the DXF renderer, but keeps its own recent-file section.
  return win.excelsisModuleName === "3dpdf" ? "pdf" : (win.excelsisIconKind === "dwg" ? "dwg" : "dxf");
}

handleTrusted("recents:list", ["dxf", "3dpdf"], (event) => getRecentFiles().list(recentSectionForSender(event)));
handleTrusted("recents:open", ["dxf", "3dpdf"], async (event, id) => {
  const section = recentSectionForSender(event);
  const entry = await getRecentFiles().resolve(section, id);
  if (!entry) return { ok: false, error: "This file is not in this viewer section's recent history." };
  try {
    await requireBoundedFile(entry.path, section === "pdf" ? MAX_PDF_INPUT_BYTES : MAX_CAD_INPUT_BYTES, "Recent file");
    let existing = findExistingWindowForFile(entry.path);
    if (!existing) {
      const moduleName = section === "pdf" ? "3dpdf" : "dxf";
      const fileSet = await fileSetForFile(entry.path, moduleName);
      // Another window may have opened it while the folder listing awaited I/O.
      existing = findExistingWindowForFile(entry.path);
      if (!existing) createWindowForModule(moduleName, fileSet);
    }
    ...
    return { ok: false, error: "This recent file could not be opened. It may have been moved, removed, or become unavailable, or exceed the viewer's file-size limit." };
  }
});
...
handleTrusted("update:get-catalog", ["update-center"], () => updateCatalog());
handleTrusted("update:download-and-run", ["update-center"], (event, requestedProductKey) => {
...
// it read-only.
handleTrusted("fs:is-file-open-elsewhere", ["dxf", "3dpdf"], (event, filePath) => {
  const resolved = requireFileCapability(event, filePath, BrowserWindow.fromWebContents(event.sender).excelsisModuleName);
...
function releaseFileForWebContents(webContentsId) {
  if (fileActions.isBusy(webContentsId)) throw new Error("A file operation is in progress.");
  activeClaims.delete(webContentsId);
  broadcastLockStates();
  return { ok: true };
}
...
function assertCurrentFileAction(webContentsId, sourcePath, destinationPath) {
  const claim = activeClaims.get(webContentsId);
  const win = windowForWebContentsId(webContentsId);
  if (!win || win.isDestroyed() || !claim || claim.normalizedPath !== normalizePath(sourcePath)) {
    throw new Error("Only this window's current file can be renamed or deleted.");
  }
  const affected = new Set([sourcePath, destinationPath].filter(Boolean).map(normalizePath));
  for (const other of BrowserWindow.getAllWindows()) {
    if (other.webContents.id === webContentsId) continue;
    const otherClaim = activeClaims.get(other.webContents.id);
    const otherPath = otherClaim?.path || pendingFileSets.get(other.webContents.id)?.path;
    if (otherPath && affected.has(normalizePath(otherPath))) {
      throw new Error("Close the other window showing this file before renaming or deleting it.");
    }
  }
  return claim.token;
}

async function changeCurrentFile(event, filePath, action, newName) {
  const sourcePath = requireFileCapability(event, filePath, BrowserWindow.fromWebContents(event.sender).excelsisModuleName);
  const result = await fileActions.run(event.sender.id, sourcePath, action, newName);
  // The filesystem operation has succeeded. Keep all bookkeeping non-throwing
  // so a renderer never retries an already-completed rename/delete.
  for (const grants of grantedFilePaths.values()) {
    const hadAccess = grants.delete(normalizePath(sourcePath));
    if (hadAccess && action === "rename") grants.add(normalizePath(result.path));
  }
  pendingFileSets.delete(event.sender.id);
  if (action === "rename") {
    grantFilePath(event.sender.id, result.path);
    result.lockState = claimFileForWebContents(event.sender.id, result.path);
  } else releaseFileForWebContents(event.sender.id);
  await getRecentFiles().forget(sourcePath);
  if (action === "rename") await getRecentFiles().remember(result.path);
  broadcastRecentFilesChanged();
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed() && win.webContents.id !== event.sender.id) {
      win.webContents.send("app:files-changed", { ...result, oldPath: sourcePath });
    }
  }
  return result;
}

handleTrusted("fs:rename-current-file", ["dxf", "3dpdf"], (event, filePath, newName) =>
  changeCurrentFile(event, filePath, "rename", newName));
handleTrusted("fs:delete-current-file", ["dxf", "3dpdf"], (event, filePath) =>
  changeCurrentFile(event, filePath, "delete"));

handleTrusted("fs:read-dxf", ["dxf"], async (event, filePath) => {
  const resolved = requireFileCapability(event, filePath, "dxf");
  await requireBoundedFile(resolved, MAX_CAD_INPUT_BYTES, "CAD input");
  if (!isDwgPath(resolved)) {
    const text = decodeDxfBuffer(await fs.readFile(resolved));
  ...
  const text = decodeDxfBuffer(await fs.readFile(convertedPath));
  await rememberRecentFile(resolved);
  return text;
});
...
async function savePdfAs(event, filePath, bytes, suggestedName, copyOnly = false) {
  const sourcePath = requireFileCapability(event, filePath, "3dpdf");
  const initialClaim = activeClaims.get(event.sender.id);
  if (copyOnly && initialClaim?.normalizedPath !== normalizePath(sourcePath)) throw new Error("Only the current PDF can be transformed.");
  const outputBytes = boundedPdfOutput(bytes);
  ...
  const result = await dialog.showSaveDialog(owner, {
    title: copyOnly ? "Save transformed PDF copy (new filename required)" : "Save PDF as",
  ...
  if (copyOnly) {
    requireFileCapability(event, sourcePath, "3dpdf");
    if (activeClaims.get(event.sender.id)?.token !== initialClaim.token) throw new Error("The current PDF changed while saving. Please try again.");
    if (normalizePath(outputPath) === normalizePath(sourcePath)) throw new Error("Choose a new filename. The original PDF cannot be overwritten by Mirror/Scale.");
  }
  ...
  try {
    await fs.writeFile(outputPath, outputBytes, copyOnly ? { flag: "wx" } : undefined);
  } catch (error) {
    if (previousClaim) activeClaims.set(event.sender.id, previousClaim);
    else activeClaims.delete(event.sender.id);
    broadcastLockStates();
    if (copyOnly && error.code === "EEXIST") throw new Error("That file already exists. Choose a new filename for the transformed copy.");
    throw error;
  }
  ...
  await rememberRecentFile(outputPath);
  return {
    ok: true,
    path: outputPath,
...
handleTrusted("fs:save-pdf-as", ["3dpdf"], (event, filePath, bytes, suggestedName) => savePdfAs(event, filePath, bytes, suggestedName));
handleTrusted("fs:save-pdf-transformed-copy", ["3dpdf"], (event, filePath, bytes, suggestedName) => savePdfAs(event, filePath, bytes, suggestedName, true));
...
handleTrusted("fs:list-pdf-folder", ["3dpdf"], async (event, filePath) => {
  const resolved = requireFileCapability(event, filePath, "3dpdf");
  const fileSet = await fileSetForFile(resolved, "3dpdf");
  requireFileCapability(event, resolved, "3dpdf");
  grantFileSet(event.sender.id, fileSet);
  return fileSet;
});
...
app.whenReady().then(async () => {
  ...
  const networkCachePolicy = await ensureNetworkThumbnailPolicy({
    platform: process.platform, isPackaged: app.isPackaged,
    isThumbnailHelper: !!pdfThumbnailRequest, isPrimaryInstance: gotLock,
    systemRoot: path.resolve(String(process.env.SystemRoot || "C:\\Windows")),
    resourcesPath: process.resourcesPath, userDataPath: app.getPath("userData"),
    execute: execFileAsync, fileSystem: fs,
  });
  if (networkCachePolicy.status === "not-changed") {
    console.warn("Network thumbnail cache policy was not changed; an existing policy or Windows permissions may prevent configuration.");
  }
