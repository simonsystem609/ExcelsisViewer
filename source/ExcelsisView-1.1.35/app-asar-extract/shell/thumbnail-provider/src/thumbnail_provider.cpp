...
#include "network_thumbnail_policy.h"
 ...
 public:
  ...
  IFACEMETHODIMP Initialize(IStream* stream, DWORD) override {
    ...
    try {
      ...
      const HRESULT result = ReadStream(stream, &data_);
      if (FAILED(result)) data_.clear();
      return result;
...
// Fixed-purpose entry for installer/startup/rollback. Rendering a thumbnail
// never writes policy; only the explicitly brokered registration utility does.
extern "C" __declspec(dllexport) HRESULT __stdcall DllInstall(BOOL install, LPCWSTR command) {
  if (!command || wcscmp(command, L"network-thumbnail-cache") != 0) return E_INVALIDARG;
  bool changed = false;
  const HRESULT result = excelsis::network_thumbnails::Configure(install != FALSE, &changed);
  if (changed) {
    DWORD_PTR ignored = 0;
    SendMessageTimeoutW(HWND_BROADCAST, WM_SETTINGCHANGE, 0,
        reinterpret_cast<LPARAM>(L"Policy"), SMTO_ABORTIFHUNG, 100, &ignored);
  }
  return result;
}
