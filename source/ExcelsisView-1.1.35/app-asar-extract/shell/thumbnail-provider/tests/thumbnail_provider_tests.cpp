...
class TrackingStream final : public IStream {
 public:
  TrackingStream(IStream* inner, bool* destroyed, HRESULT readFailure)
      : inner_(inner), destroyed_(destroyed), readFailure_(readFailure) {
 ...
 private:
  ...
  HRESULT readFailure_ = S_OK;
};

IStream* CreateTrackingStream(const std::vector<std::uint8_t>& bytes, bool* destroyed, HRESULT readFailure) {
  ...
  return new TrackingStream(inner, destroyed, readFailure);
}
...
IThumbnailProvider* CreateProvider(
  ...
  IStream* stream = CreateTrackingStream(bytes, &sourceStreamDestroyed, readFailure);
...
void TestGeometry(DllGetClassObjectFunction getClassObject, const wchar_t* outputPath) {
  ...
  HBITMAP repeated = nullptr;
  WTS_ALPHATYPE repeatedAlpha = WTSAT_UNKNOWN;
  Check(SUCCEEDED(provider->GetThumbnail(128, &repeated, &repeatedAlpha)) && repeated != nullptr,
      "repeated thumbnail call after source stream release");
  Check(InspectBitmap(repeated, 128).darkPixels > 100, "repeated thumbnail content");
  DeleteObject(repeated);
  provider->Release();
...
void TestFailedAndCancelledRead(DllGetClassObjectFunction getClassObject) {
  const auto source = Bytes("0\nSECTION\n2\nENTITIES\n0\nLINE\n10\n0\n20\n0\n11\n10\n21\n10\n0\nENDSEC\n0\nEOF\n");
  for (HRESULT failure : {STG_E_READFAULT, HRESULT_FROM_WIN32(ERROR_CANCELLED)}) {
    HRESULT result = S_OK;
    IThumbnailProvider* provider = CreateProvider(getClassObject, source, &result, failure);
    Check(result == failure, "failed/cancelled source read propagated with source already released");
    for (int attempt = 0; attempt < 2; ++attempt) {
      HBITMAP bitmap = nullptr;
      WTS_ALPHATYPE alpha = WTSAT_UNKNOWN;
      Check(provider->GetThumbnail(128, &bitmap, &alpha) == E_UNEXPECTED && bitmap == nullptr,
          "failed initialization never renders partial buffered data");
    }
    provider->Release();
  }
}
...
#include "network_thumbnail_policy_tests.h"
...
int wmain(int argc, wchar_t** argv) {
  ...
  TestFailedAndCancelledRead(getClassObject);
  ...
  TestNetworkThumbnailPolicy(module);
