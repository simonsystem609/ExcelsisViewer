...
import { askRacetrackOptions } from "./racetrack-dialog.mjs";
import { commonSelectionSizing, parseSelectionNumber, stepSelectionNumber } from "./selection-sizing.mjs";
...
const ui = {
  ...
  rotateSplit: document.getElementById("rotateSplit"),
  rotateBtn: document.getElementById("rotateBtn"),
  rotateMenuBtn: document.getElementById("rotateMenuBtn"),
  rotateMenu: document.getElementById("rotateMenu"),
  rotateDegreeBtn: document.getElementById("rotateDegreeBtn"),
  rotateLineBtn: document.getElementById("rotateLineBtn"),
  rotateDegreeDialog: document.getElementById("rotateDegreeDialog"),
  rotateDegreeInput: document.getElementById("rotateDegreeInput"),
  rotateDegreeApplyBtn: document.getElementById("rotateDegreeApplyBtn"),
  rotateDegreeCancelBtn: document.getElementById("rotateDegreeCancelBtn"),
  ...
  racetrackBtn: document.getElementById("racetrackBtn"),
...
function racetrackMetrics(feature) {
  ...
  const polylineArcs = ents.length === 1 && ents[0].type === "LWPOLYLINE"
    ? polylineSegments(ents[0]).filter((segment) => Math.abs(segment.bulge) >= BULGE_TOL)
      .map((segment) => bulgeToArc(segment.a, segment.b, segment.bulge))
    : [];
  if (polylineArcs.length === 2 && polylineArcs.every(Boolean)) {
    const r = (polylineArcs[0].r + polylineArcs[1].r) / 2;
    const centerDistance = distance(
      { x: polylineArcs[0].cx, y: polylineArcs[0].cy },
      { x: polylineArcs[1].cx, y: polylineArcs[1].cy },
    );
    return { length: centerDistance + 2 * r, radius: r, arcBased: false };
  }
...
function renderCanvasNow() {
  ...
  for (const pick of state.racetrackPicks) drawPointMarker(pick.point, "#f5cb7e", 6);
  if (state.racetrackHover) {
    drawPointMarker(state.racetrackHover.point, SNAP_COLOR, 5);
    if (state.racetrackPicks.length) drawLineReference({ start: state.racetrackPicks[0].point, end: state.racetrackHover.point }, SNAP_COLOR, 1);
  }
...
function drawLineReference(reference, color, width) {
...
function pickRacetrackPoints() {
  return new Promise((resolve) => {
    state.racetrackPicks = [];
    state.racetrackHover = null;
    state.hoverSnap = null;
    state.selectionBox = null;
    pendingHoverPoint = null;
    canvas.classList.add("picking-racetrack");
    const prompt = () => { ui.hud.textContent = `Racetrack: pick ${state.racetrackPicks.length ? "second" : "first"} end center. Snap to a vertex / circle center or click any point. Pan / zoom as needed. Esc cancels.`; };
    const candidate = (event) => {
      const screen = pointerPos(event);
      const candidates = [];
      for (const entity of nearbyEntities(screen, 15)) addEntitySnapCandidates(candidates, entity, screen);
      let best = null;
      for (const snap of candidates) {
        const p = worldToScreen(snap.point);
        const distance = Math.hypot(p.x - screen.x, p.y - screen.y);
        const score = distance - (snap.priority || 0);
        if (distance <= 15 && (!best || score < best.score)) best = { ...snap, score };
      }
      return best || { point: screenToWorld(screen), label: "point" };
    };
    const finish = (result) => {
      canvas.removeEventListener("mousemove", move, true);
      canvas.removeEventListener("mousedown", blockLeft, true);
      canvas.removeEventListener("mouseup", blockLeft, true);
      ...
      state.hoverSnap = null;
      ui.snapBadge.style.display = "none";
      canvas.classList.remove("picking-racetrack");
      render();
      resolve(result);
    };
    const move = (event) => {
      ...
      event.stopImmediatePropagation();
      const pick = candidate(event);
      state.racetrackHover = pick;
      ui.snapBadge.style.display = "block";
      ui.snapBadge.textContent = `${pick.label}: ${fmt(pick.point.x)}, ${fmt(pick.point.y)}`;
      render();
    };
    const blockLeft = (event) => {
      if (event.button !== 0) return;
      event.preventDefault();
      event.stopImmediatePropagation();
    };
    const click = (event) => {
      ...
      blockLeft(event);
      const pick = candidate(event);
      const first = state.racetrackPicks[0]?.point;
      if (first && Math.hypot(first.x - pick.point.x, first.y - pick.point.y) < 1e-6) {
        ui.hud.textContent = "The two end centers must be different. Pick the second point; Esc cancels.";
        return;
      }
      state.racetrackPicks.push({ ...pick, point: { x: pick.point.x, y: pick.point.y } });
      if (state.racetrackPicks.length === 2) finish([...state.racetrackPicks]);
      else { prompt(); render(); }
    };
    const key = (event) => {
      event.stopImmediatePropagation();
    ...
    canvas.addEventListener("mousedown", blockLeft, true);
    canvas.addEventListener("mouseup", blockLeft, true);
...
async function connectWithRacetrack() {
  if (!state.doc || state.readOnly || state.racetrackBusy) return;
  const originalDoc = state.doc;
  state.racetrackBusy = true;
  // Freeze document controls during picking. The canvas still allows pan/zoom.
  const controls = [...document.querySelectorAll(".toolbar, .panel, .pager, #recentsButton")];
  const wasInert = controls.map((element) => element.inert);
  controls.forEach((element) => { element.inert = true; });
  let message = "";
  syncUi();
  try {
    const picks = await pickRacetrackPoints();
    if (!picks) return;
    const plan = await askRacetrackOptions(picks);
    if (!plan) return;
    if (state.doc !== originalDoc || state.readOnly) throw new Error("The drawing changed while choosing the racetrack. Please try again.");
    const template = picks.find((pick) => pick.entity)?.entity;
    const insert = findEntitiesInsertIndex();
    const entity = {
      id: state.doc.entities.reduce((maximum, item) => Math.max(maximum, item.id || 0), 0) + 1,
      type: "LWPOLYLINE", originalType: "LWPOLYLINE", start: insert, end: insert - 1,
      layer: template?.layer || "0", space: state.doc.activeSpace,
      deleted: false, modified: true, featureId: null, supported: true,
      racetrackGenerated: true, closed: true, points: plan.points,
    };
    entity.pairs = buildLwPolylinePairs(entity);
    const style = generatedEntityStylePairs(template);
    if (!style.some((pair) => pair.code === "67") && entity.space === DXF_PAPER_SPACE) style.push({ code: "67", value: "1" });
    entity.pairs.splice(2, 1, ...style);
    pushUndoSnapshot();
    state.doc.entities.push(entity);
    state.selectedEntityIds.clear();
    state.selectedFeatureIds.clear();
    ...
    if (feature && isGroupedFeature(feature)) state.selectedFeatureIds.add(feature.id);
    else state.selectedEntityIds.add(entity.id);
    updateDirty();
    message = `Added ${plan.kind} racetrack: width ${fmt(plan.width)} mm, end radius ${fmt(plan.endRadius)} mm${plan.kind === "curved" ? `, curve radius ${fmt(plan.curveRadius)} mm` : ""}. Save to keep it; Ctrl+Z undoes it.`;
  } catch (error) {
    console.error(error);
    alert(`Connect with racetrack failed: ${error.message || error}`);
  } finally {
    state.racetrackBusy = false;
    controls.forEach((element, index) => { element.inert = wasInert[index]; });
    syncUi();
    render();
    if (message) ui.hud.textContent = message;
  }
}
...
function captureRotationInteraction() {
  return {
    mode: state.mode,
    selectedEntityIds: new Set(state.selectedEntityIds),
    selectedFeatureIds: new Set(state.selectedFeatureIds),
  };
}

function restoreRotationInteraction(snapshot) {
  if (!snapshot) return;
  state.mode = snapshot.mode === "measure" ? "measure" : "select";
  state.selectedEntityIds.clear();
  for (const id of snapshot.selectedEntityIds || []) {
    if (entityById(id)) state.selectedEntityIds.add(id);
  }
  state.selectedFeatureIds.clear();
  for (const id of snapshot.selectedFeatureIds || []) {
    if (featureById(id)) state.selectedFeatureIds.add(id);
  }
}

function normalizedSignedRotationDegrees(value) {
...
function rotationDirectionLabel(angleDegrees) {
  const absolute = Math.abs(angleDegrees);
  return `${fmt(absolute)} degree${Math.abs(absolute - 1) < 1e-9 ? "" : "s"} ${angleDegrees < 0 ? "clockwise" : "counter-clockwise"}`;
}

function rotateCurrentDrawingByAngle(requestedAngleDegrees, interactionSnapshot = captureRotationInteraction()) {
  if (!state.doc || state.rotateBusy || !canRotateCurrentDrawing()) return;
  const angleDegrees = normalizedSignedRotationDegrees(requestedAngleDegrees);
  if (!Number.isFinite(angleDegrees)) {
    alert("Rotate failed: enter a valid rotation angle.");
    return;
  }
  if (angleDegrees === 0) {
    restoreRotationInteraction(interactionSnapshot);
    syncUi();
    render();
    ui.hud.textContent = "The selected rotation is already aligned; no change was needed.";
    return;
  }
  ...
  for (const entity of candidates) {
    if (rotateEntityInPlace(entity, center, angleDegrees)) rotated++;
  }
  ...
  ui.hud.textContent = `Rotated ${rotationDirectionLabel(angleDegrees)} around the drawing center.${outputHint}`;
}

function rotateCurrentDrawing() {
  setRotateMenuOpen(false);
  rotateCurrentDrawingByAngle(CLOCKWISE_QUARTER_TURN_DEGREES);
}

function askRotationDegrees() {
  return new Promise((resolve) => {
    if (!ui.rotateDegreeDialog || !ui.rotateDegreeInput) {
      resolve(null);
      return;
    }
    const form = ui.rotateDegreeDialog.querySelector("form");
    ui.rotateDegreeInput.value = "90";

    const finish = (value) => {
      cleanup();
      if (ui.rotateDegreeDialog.open) ui.rotateDegreeDialog.close();
      resolve(value);
    };
    const onSubmit = (event) => {
      event.preventDefault();
      if (!ui.rotateDegreeInput.reportValidity()) return;
      const degrees = Number(ui.rotateDegreeInput.value);
      if (!Number.isFinite(degrees) || Math.abs(degrees) > 360) {
        ui.rotateDegreeInput.setCustomValidity("Enter a value from -360 to 360.");
        ui.rotateDegreeInput.reportValidity();
        ui.rotateDegreeInput.setCustomValidity("");
        return;
      }
      finish(degrees);
    };
    ...
    const cleanup = () => {
      form?.removeEventListener("submit", onSubmit);
      ui.rotateDegreeCancelBtn?.removeEventListener("click", onCancel);
      ui.rotateDegreeDialog.removeEventListener("cancel", onDialogCancel);
    };
    form?.addEventListener("submit", onSubmit);
    ui.rotateDegreeCancelBtn?.addEventListener("click", onCancel);
    ui.rotateDegreeDialog.addEventListener("cancel", onDialogCancel);
    ui.rotateDegreeDialog.showModal();
    requestAnimationFrame(() => ui.rotateDegreeInput.select());
  });
}

async function rotateCurrentDrawingByDegree() {
  setRotateMenuOpen(false);
  const clockwiseDegrees = await askRotationDegrees();
  if (clockwiseDegrees == null) return;
  rotateCurrentDrawingByAngle(clockwiseInputToDxfDegrees(clockwiseDegrees));
}

async function rotateCurrentDrawingByLine() {
  setRotateMenuOpen(false);
  if (!state.doc || state.rotateBusy || !canRotateCurrentDrawing()) return;
  const interactionSnapshot = captureRotationInteraction();
  const reference = await pickLineReference({
    prompt: "Click a straight line or straight polyline segment to align it with X. Press Esc to cancel.",
  });
  if (!reference) return;
  const angleDegrees = rotationDegreesToAlignXAxis(reference);
  if (!Number.isFinite(angleDegrees)) {
    alert("Rotate by line failed: the selected line has no usable direction.");
    return;
  }
  rotateCurrentDrawingByAngle(angleDegrees, interactionSnapshot);
}
...
// Temporarily takes over canvas input to pick a raw LINE or a straight segment
// inside an undissolved LWPOLYLINE. Capture-phase handlers keep normal contour
// selection and measurement from changing while the picker is live.
function pickLineReference({ prompt, dialogToRestore = null } = {}) {
  return new Promise((resolve) => {
    const interactionSnapshot = captureRotationInteraction();
    state.pickingLine = true;
    state.pickHoverLineReference = null;
    canvas.classList.add("picking-line");
    ui.hud.textContent = prompt || "Click a straight line. Press Esc to cancel.";
    const restoreDialog = !!dialogToRestore?.open;
    if (restoreDialog) dialogToRestore.close();
    render();
    ...
    const finish = (reference) => {
      ...
      restoreRotationInteraction(interactionSnapshot);
      if (restoreDialog) dialogToRestore.showModal();
      syncUi();
      render();
    ...
    const onMove = (event) => {
      ...
      state.pickHoverLineReference = nearestLineReference(p);
      render();
    };
    const blockPointer = (event) => {
      event.preventDefault();
    ...
    const onClick = (event) => {
      ...
      event.stopImmediatePropagation();
      const p = pointerPos(event);
      const reference = nearestLineReference(p);
      if (reference) finish(reference);
    };
    const onKey = (event) => {
      if (event.key === "Escape") {
        event.preventDefault();
...
function pickLineForScale() {
  return pickLineReference({
    prompt: "Click a straight line or straight polyline segment to use as the scale reference. Press Esc to cancel.",
    dialogToRestore: ui.scaleDialog,
  });
}
...
function askScaleOptions() {
  return new Promise((resolve) => {
    ...
    const onPickLine = async () => {
      const reference = await pickLineForScale();
      if (!reference) return;
      const length = Math.hypot(reference.end.x - reference.start.x, reference.end.y - reference.start.y);
      pickedLine = { reference, length };
...
function transformSelectedContours(fn) {
  transformSelectedFeatures(fn, (feature) => isContourFeature(feature) && canOffsetFeature(feature));
}
...
function setRacetrackGeometry(feature, length, radius) {
  ...
  const arcs = entities.filter((e) => e.type === "ARC");
  if (arcs.length !== 2) {
    const polyline = entities.length === 1 && isPolylineRacetrack(entities[0]) ? entities[0] : null;
    if (!polyline) return;
    const caps = polylineSegments(polyline)
      .filter((segment) => Math.abs(segment.bulge) >= BULGE_TOL)
      .map((segment) => bulgeToArc(segment.a, segment.b, segment.bulge));
    if (caps.length !== 2 || caps.some((cap) => !cap)) return;
    radius = Math.max(0.001, Math.min(radius, length / 2 - 0.001));
    const oldRadius = (caps[0].r + caps[1].r) / 2;
    const dx = caps[1].cx - caps[0].cx;
    const dy = caps[1].cy - caps[0].cy;
    const oldDistance = Math.hypot(dx, dy);
    if (oldRadius <= 0 || oldDistance <= 1e-9) return;
    const ux = dx / oldDistance;
    const uy = dy / oldDistance;
    const center = { x: (caps[0].cx + caps[1].cx) / 2, y: (caps[0].cy + caps[1].cy) / 2 };
    const lengthScale = (length - 2 * radius) / oldDistance;
    const widthScale = radius / oldRadius;
    for (const point of polyline.points) {
      const x = point.x - center.x;
      const y = point.y - center.y;
      const along = (x * ux + y * uy) * lengthScale;
      const across = (-x * uy + y * ux) * widthScale;
      point.x = center.x + along * ux - across * uy;
      point.y = center.y + along * uy + across * ux;
    }
    return;
  }
...
function nearestLineReference(screenPoint) {
  if (!state.doc) return null;
  let best = null;
  for (const entity of nearbyEntities(screenPoint, HIT_PX + 2)) {
    for (const reference of lineReferencesForEntity(entity)) {
      const distance = distPointSegment(
        screenPoint,
        worldToScreen(reference.start),
        worldToScreen(reference.end),
      );
      if (distance <= HIT_PX && (!best || distance < best.distance)) {
...
function syncUi() {
  ...
  ui.prevBtn.disabled = state.fileNavigationBusy || state.fileIndex <= 0;
  ui.nextBtn.disabled = state.fileNavigationBusy || !state.files.length || state.fileIndex >= state.files.length - 1;
  ...
  const fileActionDisabled = !state.doc || !current?.path || !desktopApi?.renameFile || state.fileActionBusy
    || (state.readOnly && !isConvertedDwg) || state.repairBusy || state.scaleBusy || state.rotateBusy
    || state.cornerTreatmentBusy || state.cleanupBusy || state.mirrorBusy || state.racetrackBusy || state.originalOverlayBusy;
  ui.renameFileBtn.disabled = fileActionDisabled;
  ui.deleteFileBtn.disabled = fileActionDisabled;
  for (const button of [ui.fixOuterContourBtn, ui.addChamferFilletBtn, ui.removeChamferFilletBtn, ui.racetrackBtn]) {
    if (button) button.hidden = isConvertedDwg;
  }
  ...
  if (ui.racetrackBtn) ui.racetrackBtn.disabled = !state.doc || state.readOnly || state.racetrackBusy || state.repairBusy || state.scaleBusy || state.rotateBusy || state.cornerTreatmentBusy || state.cleanupBusy || state.mirrorBusy;
  if (ui.rotateBtn) ui.rotateBtn.disabled = !state.doc || state.rotateBusy || !canRotateCurrentDrawing();
  if (ui.rotateMenuBtn) ui.rotateMenuBtn.disabled = !state.doc || state.rotateBusy || !canRotateCurrentDrawing();
  if (ui.rotateDegreeBtn) ui.rotateDegreeBtn.disabled = !state.doc || state.rotateBusy || !canRotateCurrentDrawing();
  if (ui.rotateLineBtn) ui.rotateLineBtn.disabled = !state.doc || state.rotateBusy || !canRotateCurrentDrawing();
  if (!state.doc) setRotateMenuOpen(false);
...
function featureSizingDescriptor(feature) {
  if (!canOffsetFeature(feature)) return null;
  const shape = featureShape(feature);
  const width = feature.bbox.maxX - feature.bbox.minX;
  const height = feature.bbox.maxY - feature.bbox.minY;
  if (shape === "hole") return { shape, values: { diameter: featureEntities(feature)[0].r * 2 } };
  if (shape === "racetrack") {
    const { length, radius } = racetrackMetrics(feature);
    return { shape, values: { length, radius } };
  }
  if (shape === "rectangle") return { shape, values: { width, height } };
  if (shape === "rounded-rectangle") {
    return { shape, values: { width, height, cornerRadius: averageArcRadius(feature) } };
  }
  return null;
}

function appendFeatureSizingRows(wrap, descriptor) {
  if (!descriptor) return;
  const { shape, values } = descriptor;
  if (shape === "hole") {
    wrap.appendChild(selectionNumberRow("Diameter", values.diameter, "Set", applySelectedFeatureSize, { unit: "mm" }));
    wrap.appendChild(selectionNumberRow("Radius", values.diameter / 2, "Set", applySelectedFeatureRadius, { unit: "mm" }));
  } else if (shape === "racetrack") {
    wrap.appendChild(selectionNumberRow("Length", values.length, "Set", applySelectedFeatureLength, { unit: "mm" }));
    wrap.appendChild(selectionNumberRow("Radius", values.radius, "Set", applySelectedFeatureRadius, { unit: "mm" }));
  } else if (shape === "rectangle" || shape === "rounded-rectangle") {
    wrap.appendChild(selectionNumberRow("Width", values.width, "Set", applySelectedFeatureWidth, { unit: "mm" }));
    wrap.appendChild(selectionNumberRow("Height", values.height, "Set", applySelectedFeatureHeight, { unit: "mm" }));
    if (shape === "rounded-rectangle") {
      wrap.appendChild(selectionNumberRow("Corner R", values.cornerRadius, "Set", applySelectedFeatureRadius, { unit: "mm" }));
    }
  }
}
...
function renderSingleContourSelection(feature) {
  ...
  const sizing = featureSizingDescriptor(feature);
  if (sizing) appendFeatureSizingRows(wrap, sizing);
  else if (shape === "open") {
    wrap.appendChild(selectionText("Open contour: offset around its center.", "selection-muted"));
  } else {
    wrap.appendChild(selectionText(canOffsetFeature(feature)
      ? "Complex closed contour: offset only."
      : "This contour cannot be edited because it belongs to a block reference.", "selection-muted"));
  }
...
function renderMultipleSelection(features, rawEntities) {
  ...
  const sharedSizing = rawEntities.length === 0 && features.every(canOffsetFeature)
    ? commonSelectionSizing(features.map(featureSizingDescriptor)) : null;
  if (sharedSizing) {
    wrap.appendChild(selectionText(`Same-size ${sharedSizing.shape.replaceAll("-", " ")} contours: set a target size for all ${features.length}.`, "selection-muted"));
    appendFeatureSizingRows(wrap, sharedSizing);
  }
...
function selectionNumberControl(value, onEnter) {
  const control = document.createElement("div");
  control.className = "selection-number-control";
  ...
  input.addEventListener("input", () => {
    ...
    const number = parseSelectionNumber(input.value);
    if (Number.isFinite(number)) input.setAttribute("aria-valuenow", String(number));
    else input.removeAttribute("aria-valuenow");
  });
  const step = (direction) => {
    const current = parseSelectionNumber(input.value);
    const next = stepSelectionNumber(Number.isFinite(current) ? current : value, direction);
    input.value = fmtInput(next);
  ...
  arrows.className = "selection-step-buttons";
  for (const [direction, text, title] of [[1, "▲", "Increase by 0.1"], [-1, "▼", "Decrease by 0.1"]]) {
...
function readSelectionNumber(input) {
  const value = parseSelectionNumber(input.value);
  if (Number.isFinite(value)) return value;
  input.setCustomValidity("Enter a valid number. Comma and dot decimals are supported.");
...
function selectionNumberRow(label, value, buttonText, onApply, { unit = "" } = {}) {
  ...
  let input;
  const apply = () => {
    const number = readSelectionNumber(input);
    if (Number.isFinite(number)) onApply(number);
  };
  const numeric = selectionNumberControl(value, apply);
  input = numeric.input;
  ...
  row.append(labelSpan, numeric.control, unitSpan, button);
  return row;
}
...
function selectionOffsetEditor(features) {
  ...
  let xInput, yInput;
  const apply = () => applySelectionAxisOffset(readSelectionNumber(xInput), readSelectionNumber(yInput));
  const xNumeric = selectionNumberControl(0, apply);
  xInput = xNumeric.input;
  xInput.setAttribute("aria-label", "X offset");
  const yLabel = document.createElement("span");
  yLabel.textContent = "Y:";
  const yNumeric = selectionNumberControl(0, apply);
  yInput = yNumeric.input;
  yInput.setAttribute("aria-label", "Y offset");
  ...
  axisRow.append(xLabel, xNumeric.control, yLabel, yNumeric.control, unit, applyButton);
...
function fmtInput(value) {
  return Number.isFinite(value) ? value.toFixed(9).replace(/\.?0+$/, "") : "0";
}
...
function buildLwPolylinePairs(e) {
  ...
  const number = e.racetrackGenerated ? (value) => String(value) : formatNumber;
  ...
  for (const p of points) {
    pairs.push({ code: "10", value: number(p.x) });
    pairs.push({ code: "20", value: number(p.y) });
    if (Math.abs(p.bulge || 0) > BULGE_TOL) {
      pairs.push({ code: "42", value: number(p.bulge) });
    }
  }
  return pairs;
}
...
function updatedPairsForEntity(e) {
  ...
  if (e.type === "LINE") {
    ...
    if (e.racetrackGenerated || e.forceRebuildPairs || e.repairedOuterContour || !pairs.length || !Number.isFinite(originalPointCount) || originalPointCount !== e.points.length) {
      return buildLwPolylinePairs(e);
    }
...
function setRotateMenuOpen(open) {
  if (!ui.rotateMenu || !ui.rotateMenuBtn) return;
  const nextOpen = !!open && !ui.rotateMenuBtn.disabled;
  ui.rotateMenu.hidden = !nextOpen;
  ui.rotateMenuBtn.setAttribute("aria-expanded", String(nextOpen));
}
...
async function refreshDxfFolderFiles() {
  ...
  if (state.fileActionBusy || currentFile()?.path !== currentPath) return;
  if (!fileSet?.files?.length) return;
  state.files = fileSet.files;
...
async function manageCurrentFile(kind) {
  const current = currentFile();
  if (!current?.path || !state.doc || state.fileActionBusy || ui.renameFileBtn.disabled) return;
  state.fileActionBusy = true;
  const originalDocument = state.doc;
  try {
    await confirmFileAction({
      kind, name: current.name, path: current.path, dirty: state.dirty,
      execute: async (newName) => {
        if (currentFile() !== current || state.doc !== originalDocument) throw new Error("The open file changed. Reopen the confirmation.");
        const result = kind === "rename"
          ? await desktopApi.renameFile(current.path, newName)
          : await desktopApi.deleteFile(current.path);
        if (!result?.ok) throw new Error(result?.error || "The file operation did not complete.");
        if (kind === "rename") {
          current.path = result.path;
          current.name = result.name;
          // Keep the document, edits, undo stack, selection and view intact.
          state.files.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: "base" }));
          state.fileIndex = state.files.indexOf(current);
          syncUi();
        } else {
          state.files = state.files.filter((file) => file !== current);
          // Clear the deleted document before trying to open another file.
          state.fileIndex = -1;
          await loadCurrentFile();
          ui.hud.textContent = `${current.name} moved to the Recycle Bin. Use Next to open another file.`;
        }
        return result;
      },
    });
  } finally {
    state.fileActionBusy = false;
    syncUi();
  }
}
...
async function goFile(delta) {
  if (!state.files.length || folderNavigationBlocked()) return;
  state.fileNavigationBusy = true;
  syncUi();
  try {
  ...
  await loadCurrentFile();
    ...
    ui.hud.textContent = `Could not change file: ${error.message || error}`;
  } finally {
    state.fileNavigationBusy = false;
    syncUi();
  }
}

function folderNavigationBlocked() {
  return state.fileNavigationBusy || state.fileActionBusy || state.racetrackBusy ||
    state.repairBusy || state.scaleBusy || state.rotateBusy || state.mirrorBusy ||
    state.cornerTreatmentBusy || state.cleanupBusy || state.originalOverlayBusy;
}
...
ui.renameFileBtn.addEventListener("click", () => manageCurrentFile("rename"));
ui.deleteFileBtn.addEventListener("click", () => manageCurrentFile("delete"));
...
if (ui.racetrackBtn) ui.racetrackBtn.addEventListener("click", connectWithRacetrack);
if (ui.rotateBtn) ui.rotateBtn.addEventListener("click", rotateCurrentDrawing);
if (ui.rotateMenuBtn) {
  ui.rotateMenuBtn.addEventListener("click", () => {
    setRotateMenuOpen(ui.rotateMenu?.hidden);
  });
}
if (ui.rotateDegreeBtn) ui.rotateDegreeBtn.addEventListener("click", rotateCurrentDrawingByDegree);
if (ui.rotateLineBtn) ui.rotateLineBtn.addEventListener("click", rotateCurrentDrawingByLine);
if (ui.rotateSplit) {
  document.addEventListener("pointerdown", (event) => {
    if (!ui.rotateSplit.contains(event.target)) setRotateMenuOpen(false);
  });
}
...
bindFolderNavigation({ previous: ui.prevBtn, next: ui.nextBtn, isBlocked: folderNavigationBlocked });
ui.prevBtn.title = "Previous file (Left arrow)";
ui.nextBtn.title = "Next file (Right arrow)";
...
window.addEventListener("keydown", (event) => {
  if (state.racetrackBusy || state.fileActionBusy) return;
  const focused = document.activeElement;
  const isEditingInput = ["INPUT", "TEXTAREA", "SELECT"].includes(focused?.tagName) || focused?.isContentEditable;
  if (isEditingInput || event.isComposing) return;
  ...
  if (event.key === "Escape") {
    ...
    if (ui.rotateMenu && !ui.rotateMenu.hidden) {
      setRotateMenuOpen(false);
      ui.rotateMenuBtn?.focus();
      return;
    }
...
async function initDesktopBridge() {
  ...
  desktopApi.onFilesChanged?.((change) => {
    const current = currentFile();
    state.files = state.files.flatMap((file) => {
      if (String(file.path).toLowerCase() !== String(change.oldPath).toLowerCase()) return [file];
      return change.deleted ? [] : [{ ...file, path: change.path, name: change.name }];
    });
    state.fileIndex = current ? state.files.indexOf(current) : -1;
    syncUi();
  });
}
